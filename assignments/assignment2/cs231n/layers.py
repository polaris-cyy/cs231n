...
def softmax_loss(x, y):
    ...
    num_train = x.shape[0]
    true_score = x[range(num_train), y].reshape(num_train, 1)
    exp_sum = np.sum(np.exp(x), axis=1).reshape(num_train, 1)
    loss = np.sum(np.log(exp_sum)-true_score)/num_train
...
def batchnorm_backward_alt(dout, cache):
    ...
    dxhat = dout * gamma
    dvar = np.sum((x - sample_mean) * dxhat * -0.5 / a ** 3, axis=0)
    dmean = np.sum(- dxhat / a, axis=0) + dvar * np.sum(-2 * (x - sample_mean), axis=0)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for j in range(out_H):
      for k in range(out_W):
        x_mask = padded_x[:, :, j * stride:j * stride + HH, k * stride:k * stride + WW]
        for i in range(F):
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(out_H):
        for j in range(out_W):
            x_mask = padded_x[:,:,i*stride:i*stride+HH,j*stride:j*stride+WW]
            # dw为dout和x_mask的卷积
            for k in range(F):
